...
xPPM (eXtended Process to Product Modeling) neo-Seoul is a desktop application for authoring Information Delivery Manuals (IDMs) according to ISO 29481 standards. It enables you to:
...
| Feature | Description |
|---------|-------------|
| **Bug Fix: Add IU on First Open** | Information Units can now be added immediately after a new ER is created from a BPMN Data Object — no longer requires closing and reopening the Individual ER pane |

### What's New in v1.4.0

| Feature | Description |
|---------|-------------|
| **mvdXML Import** | Import buildingSMART Model View Definition (mvdXML 1.1) files; ConceptTemplates, ConceptRoots, and TemplateRules are converted to ERs and Information Units |
| **mvdXML Export** | Export IDM Exchange Requirements as mvdXML 1.1; files imported from mvdXML are re-exported with faithful round-trip fidelity (original templates, rules, and ExchangeRequirements preserved verbatim) |
...
| buildingSMART mvdXML 1.1 | Model View Definition |

### Supported Platforms

| Platform | File | Notes |
|----------|------|-------|
| macOS (Apple Silicon) | `xPPM neo-Seoul-arm64.dmg` | M1/M2/M3/M4 Macs |
| macOS (Intel) | `xPPM neo-Seoul-x64.dmg` | Intel-based Macs |
| Windows | `xPPM neo-Seoul Setup x.x.x.exe` | 64-bit Windows |
| Linux | `xPPM neo-Seoul-x.x.x.AppImage` | 64-bit Linux |
...
When you launch xPPM, you'll see the **Startup Screen** with the following options:
...
|--------|-----------|-------------|
| xPPM Project | `.idm` | Full project with BPMN diagram and all ER data |
...
| mvdXML | `.mvdxml`, `.xml` | Model View Definition (buildingSMART mvdXML 1.1) — import and export |
...
| **mvdXML (.mvdxml)** | buildingSMART Model View Definition — faithful round-trip if imported from mvdXML |
...
The IDS (Information Delivery Specification) format is a buildingSMART standard for specifying IFC model validation requirements. xPPM can export Exchange Requirements as IDS files that can be used with IFC validation tools.
...
The LOIN (Level of Information Need) format follows EN 17412 / ISO 7817-1 for specifying what information is needed for specific object types at specific project milestones. xPPM maps IDM data to LOIN by extracting context from the Use Case and grouping Information Units by their IFC entity mappings.
...
The mvdXML (Model View Definition) format is a buildingSMART standard for defining validation rules over IFC models. xPPM exports Exchange Requirements as mvdXML 1.1.

#### Round-Trip Export (Imported from mvdXML)

If you import an mvdXML file and then re-export as mvdXML, the output is semantically identical to the original. The following are all preserved exactly:

- **ConceptTemplate definitions** — the full `<Templates>` block is re-emitted verbatim, including deeply chained templates that traverse IFC inverse relationships (e.g., `HasAssociations → IfcRelAssociatesMaterial → RelatingMaterial → IfcMaterialLayerSet → MaterialLayers → IfcMaterialLayer → Material → IfcMaterial → HasProperties`)
- **TemplateRules XML** — the complete nested `<TemplateRules>` tree for each Concept is re-emitted verbatim, including:
  - Cardinality constraints (`MaterialRelation[Size]==1`, `MaterialLayers[Size]>=1`)
  - Existence checks (`LayerSetName[Exists]=TRUE`, `Material[Exists]=TRUE`)
  - Type checks (`RelatingMaterialLayerSetUsage[Type]='IfcMaterialLayerSetUsage'`)
  - Numeric value predicates (`NominalValue[Value] > '0.0'`)
  - Conjunctive binding that keeps all conditions scoped to the same root entity
- **ExchangeRequirements** — UUIDs, names, and applicability attributes restored exactly; no extra entries added
- **ConceptRoot and Concept UUIDs** — preserved from the original file

For faithful round-trip export, the file must have been imported from mvdXML in the current session or loaded from a `.idm` project file that was originally imported from mvdXML with v1.4.0 or later.

#### Synthetic Export (Created from Scratch)

If your IDM was not imported from mvdXML, the exporter constructs mvdXML synthetically from IFC external element mappings on Information Units.

> **Warning**: Synthetic export produces a structurally valid but semantically weaker mvdXML file. The output is **not equivalent** to a hand-authored mvdXML that expresses the same intent. Specifically:
>
> - IFC traversal uses the generic property-set path (`IsDefinedBy → IfcRelDefinesByProperties → IfcPropertySet`) regardless of whether the original intent was to traverse material properties (`IfcMaterial.HasProperties → IfcMaterialProperties`) or another association mechanism
> - Cardinality constraints, existence checks, and type checks are not generated
> - Numeric value predicates (`> 0.0`, `>= 1`, etc.) are not generated
> - Each IFC entity becomes an independent `ConceptRoot`; conjunctive conditions that bind multiple checks to the same instance cannot be expressed
>
> Synthetic export is suitable for simple property-existence checks on regular psets. For complex structural validation rules, author the mvdXML directly and import it into xPPM.

#### mvdXML Export Options

In the Save & Export dialog, you can choose the file extension:

- `.mvdxml` (recommended for mvdXML files)
- `.xml` (for compatibility with tools that expect `.xml`)
...
xPPM can optionally connect to a self-hosted server for centralized spec storage and multi-user collaboration. The server is **fully optional** — the app works 100% offline with local file save/load.
...
| `.mvdxml` / `.xml` (mvdXML) | Model View Definition → ER hierarchy with IUs; templates and rules preserved for round-trip |
...
mvdXML (Model View Definition) 1.1 files from buildingSMART can be imported to create an IDM specification. mvdXML defines IFC model validation rules using ConceptTemplates, ConceptRoots, and TemplateRules.

When importing mvdXML:

#### Structure Mapping

| mvdXML Element | IDM Element | Notes |
|----------------|-------------|-------|
| `<mvdXML name>` | Short Title / IDM name | Top-level file name |
| `<ModelView name>` | Full Title | View name becomes the specification title |
| `<ModelView author>` | Author | Parsed as Person (Family, Given) or Organization |
| `<ExchangeRequirement>` | Stored as metadata | Preserved verbatim for round-trip; not shown as structural sub-ERs |
| `<ConceptRoot name>` | Root ER name (prefixed `er_`) | e.g., `er_Test for Material properties of IfcBuildingElement` |
| `<ConceptRoot applicableRootEntity>` | IFC entity external mapping on root ER | Used to identify the ConceptRoot on re-export |
| `<Concept name>` | Sub-ER name | Each Concept becomes a sub-ER of the ConceptRoot ER |
| `<TemplateRules>` | Information Units (hierarchical) | Nested rules → IU hierarchy via "attach to preceding leaf" |
| `<TemplateRule Parameters>` | IU Definition (and external mapping) | Parameter string stored in IU definition for visibility in the UI |
| `<Templates>` | Preserved verbatim | Stored internally; re-emitted unchanged on mvdXML export |

#### TemplateRules → IU Conversion

Each `<TemplateRule Parameters="...">` becomes an Information Unit. The parameter string (e.g., `PropertySetName[Value]='Pset_MaterialThermal' and SimplePropertyName[Value]='ThermalConductivity' and NominalValue[Value] > '0.0'`) is stored in the IU's **Definition** field so the rules are visible in the ER Panel.

Nested `<TemplateRules>` blocks become **sub-IUs of the preceding leaf IU** — preserving the original hierarchical intent without creating artificial group containers.

When a parameter string encodes a Pset property path (e.g., `PsetName[Value]='Pset_MaterialThermal'` and `PropName[Value]='ThermalConductivity'`), an IFC external element mapping (`Pset_MaterialThermal.ThermalConductivity`) is automatically created.

#### Single vs Multiple ConceptRoots

- **Single ConceptRoot**: Becomes the root ER directly (e.g., `er_Test for Material properties of IfcBuildingElement`)
- **Multiple ConceptRoots**: Each becomes a sub-ER; a wrapper root ER named after the ModelView is created

#### Round-Trip Fidelity

For faithful round-trip back to mvdXML, the original `<Templates>` section and per-Concept `<TemplateRules>` XML are preserved internally. On re-export, the following are all restored exactly:

- **ConceptTemplate definitions** — the full IFC traversal path (e.g., `HasAssociations → IfcRelAssociatesMaterial → … → IfcMaterial → HasProperties`) is preserved; no substitution with a generic pset template
- **TemplateRules** — the complete nested rule tree including cardinality constraints (`[Size]==1`, `[Size]>=1`), existence checks (`[Exists]=TRUE`), type checks (`[Type]='...'`), and numeric value predicates (`NominalValue[Value] > '0.0'`)
- **Conjunctive binding** — conditions that must hold simultaneously for the same root entity remain bound in a single Concept; they are not split into independent ConceptRoots
- **ExchangeRequirement UUIDs, names, and applicability** — restored exactly; no extra entries added
- **ConceptRoot and Concept UUIDs** — preserved from the original file

> **Important**: Round-trip fidelity requires re-importing the original mvdXML file with v1.4.0 or later. Opening an older `.idm` project file saved before v1.4.0 will not contain the internal mvdXML metadata and will fall back to synthetic export mode, which loses the IFC traversal path, all constraints, and conjunctive binding.
...
#### mvdXML Export Looks Different from Original

**Problem**: After importing an mvdXML file and re-exporting, the output differs from the original (flat templates, extra ConceptRoots, wrong ExchangeRequirements).

**Solution**:
- This occurs when the `.idm` project file was saved before v1.4.0, which did not preserve the internal mvdXML metadata
- **Fix**: Re-import the original `.mvdxml` file (do not open the old `.idm`) — the app will then use the new import code and store the necessary metadata for round-trip export
- If re-importing from the original file, make sure to reload the app (`Ctrl+R` in the renderer window when running in dev mode) to ensure the latest code is active

### Getting Help

- **Issues**: [github.com/ghanglee/xPPM/issues](https://github.com/ghanglee/xPPM/issues)
...
*xPPM neo-Seoul is developed by Ghang Lee at the Building Informatics Group, Yonsei University.*
